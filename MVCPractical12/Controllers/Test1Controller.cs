    ...
    public class Test1Controller : Controller
    {
        ...
        public ActionResult Query1(Employee emp)
        {
            if(ModelState.IsValid)
            {
                ...
                var cmd = new SqlCommand($"INSERT INTO Employee VALUES ('{emp.FirstName}', '{emp.MiddleName}', '{emp.LastName}', '{DateTime.Parse(emp.DOB.ToString())}', '{emp.MobileNumber}', '{emp.Address}')", con);
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
