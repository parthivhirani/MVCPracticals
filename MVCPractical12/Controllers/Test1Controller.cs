...
namespace MVCPractical12.Controllers
{
    public class Test1Controller : Controller
    {
        ...
        [HttpPost]
        public ActionResult Query1(Employee emp)
        {
            if(ModelState.IsValid)
            {
                var con = new SqlConnection("data source=.; database=MVCPracticals; user id=parthiv; password=Rmha@12345678;");
                var cmd = new SqlCommand($"INSERT INTO Employee VALUES ('{emp.FirstName}', '{emp.MiddleName}', '{emp.LastName}', '{emp.DOB.Year}-{emp.DOB.Month}-{emp.DOB.Day}', '{emp.MobileNumber}', '{emp.Address}')", con);
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
